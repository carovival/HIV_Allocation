...
#load the LE computed for each facility using POC
aj= pd.read_excel('a_LEPOC.xlsx',header=None) 
aj=np.array(aj)

#load the LE computed for each facility using LAB 
rj= pd.read_excel('r_LELAB.xlsx',header=None) #LE when tested with LAB
rj=np.array(rj)
...
#O#Objective max LE
m.setObjective(quicksum(select[j] * aj[0,j]  for j in range(num_facilities)) + 
               quicksum((1-select[j])  * rj[0,j]  for j in range(num_facilities)), GRB.MAXIMIZE)
